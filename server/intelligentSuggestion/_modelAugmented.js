...
const augmentImage = (image) => {
    ...
    let augmentedImage = batchedImage;

    const numRotations = Math.floor(Math.random() * 4);
    for (let i = 0; i < numRotations; i++) {
        augmentedImage = tf.transpose(augmentedImage, [0, 2, 1, 3]);
    ...
    augmentedImage = tf.add(augmentedImage, delta);

    const contrastFactor = (Math.random() * 0.6) + 0.7;
    const mean = tf.mean(augmentedImage);
    augmentedImage = tf.sub(augmentedImage, mean);
    augmentedImage = tf.mul(augmentedImage, contrastFactor);
    augmentedImage = tf.add(augmentedImage, mean);

    return augmentedImage.squeeze([0]);
};
...
const loadDataset = async () => {
    ...
    for (let i = 0; i < productTypes.length; i++) {
        ...
        for (const item of products) {
            ...
            const augmentedImg1 = augmentImage(processedImg1);
            const augmentedImg2 = augmentImage(processedImg2);

            data.push(augmentedImg1);
            data.push(augmentedImg2);
            img1.dispose();
            img2.dispose();
            labels.push(label);
            labels.push(label);
